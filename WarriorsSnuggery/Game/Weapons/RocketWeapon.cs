...
namespace WarriorsSnuggery.Objects
{
	class RocketWeapon : Weapon
	{
		...
		{
			TargetActor = world.Actors.FirstOrDefault(a => (a.Position - target).FlatDist < 1024);
		}

		public RocketWeapon(World world, WeaponType type, Actor origin, CPos target) : base(world, type, origin, target)
		{
			TargetActor = world.Actors.FirstOrDefault(a => (origin != null && a.Team != origin.Team) && (a.Position - target).FlatDist < 1024);
		}
