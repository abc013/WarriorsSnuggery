...
namespace WarriorsSnuggery.UI.Screens
{
	public class PieceSelectionScreen : Screen
	{
		...
		public PieceSelectionScreen(Game game) : base("Piece Selection")
		{
			...
			{
				mapSelection.Add(new PanelItem(new BatchObject(UITextureManager.Get("UI_map")[0]), new MPos(512, 512), piece.Name, new[] { Color.Grey + "[" + piece.Size.X + "," + piece.Size.Y + "]" },
				() =>
				{
					GameController.CreateNew(GameSaveManager.DefaultStatistic.Copy(), MissionType.TEST, InteractionMode.EDITOR, custom: MapType.FromPiece(piece));
					Hide();
				}));
			}
			Add(mapSelection);
